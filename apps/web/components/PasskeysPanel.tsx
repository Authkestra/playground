  ...
  return (
    <Card>
      ...
      <CardContent className="flex flex-col gap-5">
        ...
        {/*
          Single column on mobile (register, horizontal rule, authenticate,
          stacked); a three-track grid from md up, where the middle track is
          `auto`-sized to exactly the vertical rule's own 1px width, giving a
          side-by-side layout with a divider between rather than a gap alone.
          Two `Separator`s, not one switching orientation, because a `md:hidden`
          element is removed from grid layout entirely — each one only ever
          occupies a track on the breakpoint where it's actually visible.
        */}
        <div className="grid grid-cols-1 gap-5 md:grid-cols-[1fr_auto_1fr] md:gap-6">
