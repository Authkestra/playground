...
/// The port `main.rs` binds when `PORT` is unset, and therefore the port every
/// deployment manifest must agree with — `EXPOSE`, `internal_port`,
/// `healthCheckPath`'s host, and so on. One constant so a future change to the
/// default cannot silently leave a manifest talking about a server that is no
/// longer there.
...
impl StarterKit {
    ...
    pub fn generate_with(
        ...
        Self {
            ...
            .chain(deploy::deploy_files(&plan))
            .collect(),
        }
    }
...
impl Plan {
    ...
    /// Every environment variable this configuration reads, in the same
    /// emission order `.env.example` renders — `PORT` and `RUST_LOG`, the
    /// credential store's `DATABASE_URL` when anything needs one, then each
    /// active scenario's own.
    ///
    /// A deployment manifest is built from this, not from a second guess at
    /// what a generated project needs — a scenario that reads
    /// `WEBAUTHN_ORIGIN` gets it declared on every host, or the manifest and
    /// the project it deploys would be free to drift apart.
    fn all_env_vars(&self) -> Vec<KitEnvVar> {
        let mut out = vec![
    ...
    fn slug(&self) -> String {
        ...
        // Docker defaults on, so it only needs to mark the name when a caller
        // turned it *off* — otherwise every ordinary download would carry
        // "-docker" for no reason. The other targets default off, so they mark
        // the name only when turned on. Either way, the name reflects
        // whatever deviates from the default, same as the two opt-ins below.
        if !self.options.deploy.docker {
            extras.push_str("-nodocker");
        }
        if self.options.deploy.render {
            extras.push_str("-render");
        }
        if self.options.deploy.fly {
            extras.push_str("-fly");
        }
        if self.options.deploy.railway {
            extras.push_str("-railway");
        }
...
{selected}
{configure}{run}{deps_section}{notes_section}{extras}{differences}{deploy_section}
## Read more
...
fn env_example(plan: &Plan) -> String {
    ...
    // `all_env_vars` is also what the deploy manifests read, so a variable
    // this configuration needs cannot appear here and be missing from one of
    // them.
...
mod tests {
    ...
    const BOTH: KitOptions = KitOptions {
        openapi: true,
        ts_client: true,
        deploy: crate::scenario::DeployTargets {
            docker: true,
            render: false,
            fly: false,
            railway: false,
